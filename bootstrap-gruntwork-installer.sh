#!/bin/bash
#
# A bootstrap script to install the Craftech Installer.
#
# Why:
#
# The goal of the Craftech Installer is to make make installing Craftech Script Modules feel as easy as installing a
# package using apt-get, brew, or yum. However, something has to install the Craftech Installer first. One option is
# for each Craftech client to do so manually, which would basically entail copying and pasting all the code below.
...
readonly FETCH_DOWNLOAD_URL_BASE="https://github.com/craftech-io/fetch/releases/download"
readonly FETCH_INSTALL_PATH="$BIN_DIR/fetch"

readonly CRAFTECH_INSTALLER_DOWNLOAD_URL_BASE="https://raw.githubusercontent.com/craftech-io/craftech-installer"
readonly CRAFTECH_INSTALLER_INSTALL_PATH="$BIN_DIR/craftech-install"
readonly CRAFTECH_INSTALLER_SCRIPT_NAME="craftech-install"

function print_usage {
  echo
  echo "Usage: bootstrap-craftech-installer.sh [OPTIONS]"
  echo
  echo "A bootstrap script to install the Craftech Installer ($CRAFTECH_INSTALLER_SCRIPT_NAME)."
  echo
  echo "Options:"
  echo
  echo -e "  --version\t\tRequired. The version of $CRAFTECH_INSTALLER_SCRIPT_NAME to install (e.g. 0.0.3)."
  ...
  echo -e "  --download-url\tOptional. The URL from where to download $CRAFTECH_INSTALLER_SCRIPT_NAME. Mostly used for automated tests. Default: $CRAFTECH_INSTALLER_DOWNLOAD_URL_BASE/(version)/$CRAFTECH_INSTALLER_SCRIPT_NAME."
  echo -e "  --no-sudo\tOptional. When true, don't use sudo to install binaries. Default: false"
  echo
  echo "Examples:"
  echo
  echo "  Install version 0.0.3:"
  echo "    bootstrap-craftech-installer.sh --version 0.0.3"
  echo
  echo "  One-liner to download this bootstrap script from GitHub and run it to install version 0.0.3:"
  echo "    curl -Ls https://raw.githubusercontent.com/craftech-io/craftech-installer/master/bootstrap-craftech-installer.sh | bash /dev/stdin --version 0.0.3"
}
...
function download_url_to_file {
  local -r url="$1"
  local -r file="$2"
  local -r tmp_path=$(mktemp "/tmp/craftech-bootstrap-download-XXXXXX")
  local -r no_sudo="$3"
...
function install_craftech_installer {
  local -r install_path="$1"
  local -r version="$2"
  local -r download_url="$3"
  local -r no_sudo="$4"

  echo "Installing $CRAFTECH_INSTALLER_SCRIPT_NAME version $version to $install_path"
...
This folder is not an industry standard, but a convention we use at  Craftech (http://www.craftech.io) make User Data
scripts manageable. Many of the modules in Script Modules (https://github.com/craftech-io/script-modules) need not
...
function bootstrap {
  ...
  if [[ -z "$download_url" ]]; then
    download_url="${CRAFTECH_INSTALLER_DOWNLOAD_URL_BASE}/${installer_version}/${CRAFTECH_INSTALLER_SCRIPT_NAME}"
  fi

  echo "Installing $CRAFTECH_INSTALLER_SCRIPT_NAME..."
  install_fetch "$FETCH_INSTALL_PATH" "$fetch_version" "$no_sudo"
  install_craftech_installer "$CRAFTECH_INSTALLER_INSTALL_PATH" "$installer_version" "$download_url" "$no_sudo"
